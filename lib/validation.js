...
/**
 * `<input type="url">` only checks URL *syntax* — it happily accepts
 * `javascript:alert(1)` or `data:text/html,...`, both valid "URLs" by that
 * measure. Founder-submitted MVP/repo links get saved as-is and later
 * rendered as `<a href>`/`<img src>` for other users (investors browsing
 * shortlists) with no scheme check, which is a stored-XSS path against
 * whoever clicks the link. Only http(s) is ever a legitimate MVP/repo URL,
 * so anything else is rejected outright rather than sanitized/rewritten.
 */
...
/** Render-time guard for any user-submitted URL used as an href/img src — see isSafeHttpUrl. */
